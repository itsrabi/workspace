...
import type { TerminalSnapshot } from "./src/domain";
import { TerminalManager, type TerminalManagerShape, type TerminalReadModel } from "./src/manager";
import {
...
import { createDeferredResultDelivery } from "./src/result-delivery";
import {
  createTerminalRuntime,
  runTool,
  type TerminalRuntime,
} from "./src/runtime";
import { sanitizeText } from "./src/ui/output-view";
import { openTerminalPicker } from "./src/ui/ps";
import { createTmuxSubagentReadModel } from "./src/tmux-subagents";
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("ps", {
    ...
    handler: async (_args, ctx) => {
      ...
      const tmuxRead = createTmuxSubagentReadModel({
        defaultCwd: ctx.cwd,
        pollMs: 1000,
        tmuxBin: "tmux",
      });

      const mergedView: TerminalReadModel = {
        list: () => [...manager.view.list(), ...tmuxRead.view.list()],
        get: (id) => manager.view.get(id) ?? tmuxRead.view.get(id),
        size: () => manager.view.size() + tmuxRead.view.size(),
        subscribe: (listener) => {
          const u1 = manager.view.subscribe(listener);
          const u2 = tmuxRead.view.subscribe(listener);
          return () => {
            u1();
            u2();
          };
        ...
        requestKill: (id) => {
          ...
          if (snap) manager.view.requestKill(id);
          else tmuxRead.view.requestKill(id);
        },
        setOnSettled: (hook) => manager.view.setOnSettled(hook),
      };

      if (manager.view.size() === 0 && tmuxRead.view.size() === 0) {
        tmuxRead.dispose();
        ctx.ui.notify(
          "No background terminals or tmux subagents yet. Start one with bg_start or run subagents.",
          "info",
        );
        return;
      }
      ...
      try {
        await openTerminalPicker(ctx, mergedView);
      } finally {
        tmuxRead.dispose();
      }
